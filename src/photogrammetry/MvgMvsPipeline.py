...
OPENMVG_BIN = Path(__file__).parent.resolve().as_posix() + "/openMVG-bin"
OPENMVS_BIN = Path(__file__).parent.resolve().as_posix() + "/openMVS-bin"

# Try to find openMVG camera sensor database
CAMERA_SENSOR_DB_FILE = "sensor_width_camera_database.txt"
CAMERA_SENSOR_DB_DIRECTORY = Path(__file__).parent.resolve().as_posix()
